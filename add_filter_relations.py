...
def filter_triples(model, tokenizer, texts):
    ...
    for batch in range(0,len(texts),range_length):
        ...
        for tensor in encoded_input:
            encoded_input[tensor] = encoded_input[tensor].to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
...
def prepare_triplet(subject_entity, object_entity, article_text, predicate):
    text_triplet = ''
    text_triplet += re.compile(r"(?<!\d)\.(?!\d)").split(article_text[:min(subject_entity['boundaries'][0], object_entity['boundaries'][0])])[-1]
    ...
    text_triplet += re.compile(r"(?<!\d)\.(?!\d)").split(article_text[max(subject_entity['boundaries'][1], object_entity['boundaries'][1]):])[0]
...
def main(folder_input = 'out/ko'):
    ...
    if language == 'he':
        model_name_or_path = 'dicta-il/dictabert'
    else:
        ...
        for k,j,y in os.walk(folder_input):
            for file_name in y:
                with jsonlines.open(k + '/' + file_name) as reader:
                    rows_parsed += 1
                    print(f"Processing {k}/{file_name}")
                    print(f"Rows parsed: {rows_parsed}, Rows filtered: {rows_filtered}")
