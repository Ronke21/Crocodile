...
def main(folder_input = 'out/ko'):
    ...
    model.eval()
    model.half()
    with jsonlines.open(f'out_clean/{"/".join(folder_input.split("/")[1:])}.jsonl', mode='w', encoding="utf-8") as writer:
        for k,j,y in os.walk(folder_input):
            for file_name in y:
                with jsonlines.open(k + '/' + file_name, encoding="utf-8") as reader:
